...
def generate_buttondown_newsletter_subject(body: str):
    ...
    sentry_count("newsletter.subject_generation.started")
    try:
        with sentry_span("ai.workflow", "newsletter.generate_subject"), sentry_duration_metric(
            "newsletter.subject_generation.duration"
        ):
            result = get_newsletter_subject_agent().run_sync(prompt)
    except Exception:
        sentry_count("newsletter.subject_generation.completed", attributes={"outcome": "failure"})
        raise

    subject = result.output
    if subject:
        sentry_count("newsletter.subject_generation.completed", attributes={"outcome": "success"})
    else:
        sentry_count("newsletter.subject_generation.completed", attributes={"outcome": "empty_response"})
        raise ValueError("No subject generated by the newsletter subject agent")
